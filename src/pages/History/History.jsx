...
const History = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        setLoading(true);
        const res = await axios.get("https://backend-speech-transcriber-2.onrender.com/history");
  ...
  const deleteHistoryItem = async (id) => {
    try {
      await axios.delete(`https://backend-speech-transcriber-2.onrender.com/history/${id}`);
